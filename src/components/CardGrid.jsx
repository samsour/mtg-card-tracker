...
const CardGrid = ({ cards, onAddCard, onRemoveCard }) => {
    ...
    return (
        <div>
            {showUndo && (
                <div style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#ffeb3b', borderRadius: '4px' }}>
                    <span>Removed card {lastRemovedCard.name}. </span>
                    <button onClick={handleUndo} style={{ padding: '5px 10px', backgroundColor: '#4CAF50', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                        Undo
                    </button>
                </div>
            )}
                ...
                {sortedCards.map((card) => (
                    <div key={card.id} style={{ display: 'flex', flexDirection: 'column', border: '1px solid #ccc', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
                        ...
                        <div style={{ padding: '10px', backgroundColor: '#f8f8f8', display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <button onClick={() => handleRemoveCard(card)} style={{ padding: '5px 10px', backgroundColor: '#ff4d4d', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                                Remove
                            </button>
                        </div>
                    </div>
